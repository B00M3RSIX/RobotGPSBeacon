...
int RoboClaw::peek()
{
	if(hserial)
		...
		return sserial->peek();
#endif
	return -1; // Return -1 if no serial available
}
...
#endif
	return 0; // Return 0 if no serial available
}
...
bool RoboClaw::ReadEncoders(uint8_t address,int32_t &enc1,int32_t &enc2){
